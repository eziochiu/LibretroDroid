...
JNIEXPORT void JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_setAudioSyncEnabled(
    JNIEnv* env,
    jclass obj,
    jboolean enabled
) {
    LibretroDroid::getInstance().setAudioSyncEnabled(enabled);
}
...
JNIEXPORT jboolean JNICALL Java_com_swordfish_libretrodroid_LibretroDroid_usesContentRenderScheduler(
    JNIEnv* env,
    jclass obj
) {
    return LibretroDroid::getInstance().usesContentRenderScheduler() ? JNI_TRUE : JNI_FALSE;
}
